...
void TwoViewGeometry::EstimateMultiple(
  ...
  int counter = 0;
  while (true) {
  	//here the multiple geometry is estimated (if this has been called, we already know we want it, so nothing to distinguish one set and two set pair is needed)
  	//maybe the number of views can be restricted to two and after it it ends
  	//in finding of the best initial pair give priority to non-multi pairs and after them try the multi pairs
  		//something that can handle this happens to occur right after this function in the function estimatewithrelativepose, which should be used for initial pair reconstruction and it wants to eat only calibrated or uncalibrated and mostly it can detect the confuguration
  		//write out the configuration when the function is called
  		//try it before calling this function from the geometry verifier in the features/matching.cc
...
void TwoViewGeometry::EstimateWithRelativePose(
  ...
  //this is called in the initial pair reconstruction
  //here might be something improved in order to cope with the multiple configuration
  EstimateCalibrated(camera1, points1, camera2, points2, matches, options);
