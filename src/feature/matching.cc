...
void TwoViewGeometryVerifier::Run() {
  ...
  while (true) {
	...
	//THRAWN
	//to je on, ten verifier
    const auto input_job = input_queue_->Pop();
    if (input_job.IsValid()) {
      auto data = input_job.Data();
      //data je typu FeatureMatcherData, obsahuje 2 id kamer, featurematches a twoviewgeometry
      //twoviewgeometry.cc a .h se nachazeji ve slozce estimators
      ...
      const auto points2 = FeatureKeypointsToPointsVector(keypoints2);
		...
		//THRAWN
		//tady to dokonce umoznuje multiple model estimation
		//DONE twoviewgeometryverifier by mel mit moznost se dozvedet s jakym typem paru zrovna pracuje, pokud s tim cross, tak se spusti estimatemultiple

		//kdyz se multimodel verification pusti na singlemodel data, tak se to normalne provede, pravdepodobne si to mysli, ze to je normalni singlemodel problem
		//config vyflusnul calibrated a planar, coz znamena, ze mezi tim existuje jen jeden model a druhy to rovnou zahodi a cely ten par se oznaci jako singlemodel par
		
		//hned behem provadeni estimate multiple vypisovat jaky je vysledek (multi, single, degen) a pak to zkusit na nejaky zmenseny mbsfm problem a rovnou s tim vypisovat i jaky je to par a zjistit jak to koresponduje s tema cross parama (pokud dobre, tak by s tim jeste taky slo neco udelat)
		//potom zde vykonavat multimodel verification tehdy, kdyz vime, ze je to cross pair a jinak vyuzit singlemodel verification
		//behem two_view_geometry/estimateWithRelativePose vyzadovat potom singlebody par pro inicialisaci
		//logicky jakmile skonci tyhle sety kamer, tak tuhle podminku vyradit, ale v tom pripade budou ty tracky nejak dobre zclusterovane a objekty se budou rekonstruovat uplne nezavisle (to bude v uplne obecnem pripade libovolne se pohybujicich objektu)
		
		//nekdy i kdyz tam je jeden model, tak to najde dva, z cehoz jeden je pravdepodobne velmi maly
		//da se: zjistovat velikost druheho modelu a empirickou hranici urcit jestli to skutecne je model nebo ne
			//ignorovat to a pak to resit az behem globalniho clusteringu
		//jeste by do toho mozna slo nafitovat nejakou geometrii a to, co ji bude splnovat se vezme, ale to by bylo potreba tu geometrii nejdriv vypocitat
		//TODO
		//vyzkouset jak se vysledek zmeni kdyz tam misto opravdoveho multimodel bude omezeni jen na dva modely
		//rovnou na to definovat novou funkci, ktera bude zrat o dva argumenty navic, coz budou ty dve cisla kamer, ktery se budou psat k tomu
		//potom volat MB verifikaci pouze na pary, ktere vedou mezi 2 sety, to se bude brat jako nepovinny argument colmapu (first set count) (nejdriv to muzeme zkusit s pevne danou hranici)
		//pri vyberu inicialniho paru se taky bude vyuzivat border, kdyz ho neco prekroci, tak se dany par hned zahodi

		//unsigned int border = (unsigned int)options_.border;
		//std::cout << "TRUMP " << data.image_id1 << " " << cache_->GetImage(data.image_id1).Name() << " " << data.image_id2 << " " << cache_->GetImage(data.image_id2).Name() << "\n";
		//int count1 = fs.count(cache_->GetImage(data.image_id1).Name());
		//int count2 = fs.count(cache_->GetImage(data.image_id2).Name());
		
      	//if (options_.multiple_models)
      	//if(1)
      	/*{
        	//std::cout << "MULTIMODEL VERIFICATION\n";
      	...
      	//else
      	{
        	//std::cout << "SINGLEMODEL VERIFICATION\n";
...
SiftFeatureMatcher::SiftFeatureMatcher(const SiftMatchingOptions& options,
  ...
  if (options_.guided_matching) {
    ...
    //tady se pridava vlakno ktere ma na praci provadet verifikaci matchu
    //to vlakno ma nekde svoje instrukce a ty jsou to, co my chceme upravovat
    //to, co se tam cpe je TwoViewGeometryVerifier
    for (int i = 0; i < num_threads; ++i) {
      verifiers_.emplace_back(new TwoViewGeometryVerifier(
...
void SiftFeatureMatcher::Match(
  ...
  for (const auto image_pair : image_pairs) {
    ...
    num_outputs += 1;
    //THRAWN
    //here should the matches be created
    //std::cout << "TRUMP " << image_pair.first << " " << image_pair.second << "\n";
    ...
    if (exists_matches) {
      ...
      //tady se to cpe do nejake queue, kterou potom obsluhuje jine vlakno
...
void ExhaustiveFeatureMatcher::Run() {
  ...
  for (size_t start_idx1 = 0; start_idx1 < image_ids.size();
	...
	//THRAWN
	//everything interesting happens directly here in this function
